...
const state = {
  selectedFilePath: '',
  sessionId: '', // always start with no session so each page load gets a new session
  streaming: false,
...
function appendAssistantTextDelta(messageID, partID, delta) {
  const entry = getOrCreateAssistantEntry(messageID)
  const previousText = entry.parts.get(partID) ?? ''
  upsertAssistantText(messageID, partID, `${previousText}${delta}`)
}

function removeAssistantPart(messageID, partID) {
  const entry = state.assistantByMessageID.get(messageID)
  if (!entry) {
    return
  }
...
function handleStreamEvent(event) {
  ...
  if (event.type === 'message.part.removed') {
    ...
    removeAssistantPart(messageID, partID)
    return
  }

  if (event.type === 'session.error') {
    appendChatMessage('system', formatSessionError(event))
    return
  }
